...
public class LCSSolver {
    ...
    public void solve() {
        ...
        for(int i = 0; i < xLength; i++) this.m[i][0] = 0;
        for(int j = 0; j < yLength; j++) this.m[0][j] = 0;

        for(int i = 0; i < xLength; i++){
            for(int j = 0; j < yLength; j++){
                if(this.x.charAt(i) == this.y.charAt(j)){
                    this.m[i + 1][j + 1] = this.m[i][j] + 1;
                }else if(this.m[i][j + 1] >= this.m[i + 1][j]){
                    this.m[i + 1][j + 1] = this.m[i][j + 1];
                }else{
                    this.m[i + 1][j + 1] = this.m[i + 1][j];
                }
            }
        }
    ...
    private String traceBack(int i, int j) {
        ...
        if(this.x.charAt(i - 1) == this.y.charAt(j - 1)){
            String solution = traceBack(i - 1, j - 1);
            return solution + this.x.charAt(i - 1);
    ...
    private static boolean isSubsequence(String z, String w) {
        ...
        int index = w.indexOf(z.charAt(0));
        if(index == -1) return false;
        return isSubsequence(z.substring(1, z.length()), w.substring(index));
    }
}
